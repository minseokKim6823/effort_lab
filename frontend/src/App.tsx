...
const arms:Strategy[]=['HIGH','LOW','ADAPTIVE'];
const armNames:Record<Strategy,string>={HIGH:'항상 high',LOW:'항상 low',ADAPTIVE:'자동 선택'};
 ...
 return <div className="app">
  ...
  <main>
   ...
   <div className="content">
    <section className="page-heading"><div><span className="eyebrow">{tab==='workspace'?'THINK JUST ENOUGH':tab==='benchmark'?'MEASURE, THEN OPTIMIZE':'BUILT ON EVIDENCE'}</span><h1>{tab==='workspace'?'필요한 만큼만 생각하도록.':tab==='benchmark'?'절약은 숫자로 확인하세요.':'무엇을 줄이고, 무엇을 지킬까.'}</h1><p>{tab==='workspace'?'작업에 맞는 effort를 고르고, 답의 품질과 실제 사용량을 확인하세요.':tab==='benchmark'?'같은 모델, 같은 문제. 세 가지 실행 전략을 공정하게 비교합니다.':'목표는 적은 토큰 자체가 아니라, 같은 품질을 더 적은 총토큰으로 얻는 것입니다.'}</p></div>{modeControl()}</section>
    {tab==='workspace'&&<>
     <section className="stats-grid">
      <Metric icon={<ArrowDownRight size={20}/>} label="최근 실험의 총토큰 절감" value={latest?.savingsPercent!=null?latest.savingsPercent.toFixed(2)+'%':'—'} note={latest?(latest.mode==='CODEX'?'Codex 실측 · 항상 high 대비':'데모 수치 · 실제 절감률 아님'):'비교 실험 후 표시됩니다'}/>
    ...
    {tab==='benchmark'&&<>
     <section className="panel experiment-setup"><div className="panel-heading"><div><span className="step">01</span><h2>비교 실험 설정</h2></div><span className="muted">같은 문제 · 같은 모델 · 새로운 세션</span></div>
      <div className="experiment-fields"><label>예제 작업 수<select value={caseCount} onChange={e=>setCaseCount(+e.target.value)} disabled={!!running}><option value={3}>3개 · 빠른 확인</option><option value={6}>6개 · 소규모 비교</option><option value={12}>12개 · 전체 예제</option></select></label><label>문제별 반복<select value={repeats} onChange={e=>setRepeats(+e.target.value)} disabled={!!running}>{[1,2,3].map(n=><option key={n} value={n}>{n}회</option>)}</select></label><label>총토큰 중단 기준<input type="number" min={1000} max={2000000} step={1000} value={budget} onChange={e=>setBudget(+e.target.value)} disabled={!!running}/></label>
      <button className="primary" disabled={busy||running||!status||budget<1000||budget>2000000} onClick={()=>act(async()=>{setSelected(null);setReport(await api<Report>('/benchmarks',{mode,repeats,caseCount,tokenBudget:budget}))})}>{running?<LoaderCircle className="spin" size={17}/>:<Play size={17}/>}비교 실험 시작</button></div>
      <p className="helper">기본 {caseCount*repeats*3}회 호출 + 자동 선택 재시도. 중단 기준은 호출 사이에 확인하므로 마지막 호출만큼 초과할 수 있습니다.</p>
     </section>
     {report?<><div className="report-heading"><div><span className={'status-dot '+(running?'pulse':'live')}/><strong>{stateNames[report.status]||report.status}</strong><Badge value={report.mode}/><span>{report.model}</span></div><div><span>{report.trials.length} / {report.plannedTrials}건</span>{running&&<button className="secondary compact" onClick={()=>act(async()=>{setReport(await api<Report>('/benchmarks/'+report.id+'/cancel',{}))})}><Square size={13}/>다음 호출부터 중단</button>}<button className="secondary compact" onClick={()=>downloadJson(report,'effort-lab-'+report.id+'.json')}><Download size={14}/>JSON 내보내기</button></div></div>
      ...
      <div className="finding"><Activity size={20}/><div><strong>{report.savingsPercent===null?'실험이 모두 끝나면 절감률을 계산합니다.':'항상 high 대비 총토큰 '+(report.savingsPercent>=0?report.savingsPercent.toFixed(2)+'% 감소':Math.abs(report.savingsPercent).toFixed(2)+'% 증가')}</strong><p>{report.comparable?'정답률과 문제별 실패를 함께 확인하세요. 소규모 예제 결과를 일반화할 수 없습니다. 입력 토큰도 달라질 수 있으므로 전체 변화만으로 effort의 효과를 단정하지 마세요.':'중단되거나 사용량이 누락된 실험은 완전한 비교로 표시하지 않습니다.'}</p></div></div>
      ...
      {selected&&<section className="panel detail-panel"><div className="panel-heading"><h2>{selectedTrials[0]?.title} · 응답 원문</h2><button className="secondary compact" onClick={()=>setSelected(null)}>닫기</button></div><p>{cases.find(c=>c.id===selected)?.prompt}</p><p className="helper">검증 정답: {cases.find(c=>c.id===selected)?.expectedAnswer}</p>{selectedTrials.map((t,i)=><div key={i}><strong>{armNames[t.execution.strategy]} · {t.repetition}회차</strong>{t.execution.attempts.map(a=><pre key={a.responseId}>{a.effort}: {a.output}</pre>)}</div>)}</section>}
...
function Method(){return <div className="method-grid"><section className="panel prose"><span className="eyebrow">01 / PROBLEM</span><h2>모든 작업에 high가 필요할까요?</h2><p>문자 변환과 복잡한 설계 문제에 같은 effort를 쓰면 불필요한 추론이 생길 수 있습니다. effort는 보통 호출하는 프로그램이 지정합니다. 이 앱이 그 선택을 담당합니다.</p><span className="eyebrow">02 / APPROACH</span><h2>선택은 가볍게, 품질은 확인하면서</h2><ol><li><strong>로컬 규칙으로 출발점을 정합니다.</strong> 변환·추출은 low, 계산·구현은 medium, 복잡한 추론 신호나 정확도 우선은 high입니다.</li><li><strong>같은 모델을 호출합니다.</strong> Codex 구독 로그인을 사용하므로 별도 API 키가 필요하지 않습니다.</li><li><strong>기준이 있을 때 답을 검증합니다.</strong> 실패하면 effort를 높여 재시도하고 모든 호출의 토큰을 합산합니다.</li></ol><p>키워드 규칙은 난이도를 완벽히 이해하지 못합니다. 이 버전은 개선 효과를 검증할 수 있는 출발점입니다.</p></section><section className="panel prose"><span className="eyebrow">03 / MEASUREMENT</span><h2>전체 비용을 숨기지 않습니다</h2><div className="formula">총토큰 = Σ(입력 + 출력)<small>최초 호출과 모든 재시도 포함</small></div><p>추론 토큰은 출력 토큰의 일부로 다루며 다시 더하지 않습니다. Codex 기본 지시문과 캐시 입력도 총토큰에 포함됩니다. 구독의 남은 한도나 결제 금액으로 환산하지 않습니다.</p><p>각 문제를 항상 high, 항상 low, 자동 선택으로 실행합니다. 순서를 순환 배치하고 정답률·응답 시간·추가 재시도를 함께 비교합니다.</p><span className="eyebrow">04 / LIMITS</span><h2>측정 결과를 읽는 기준</h2><ul><li>12개 예제는 기능 검증용입니다. 실제 업무 전체를 대표하지 않습니다.</li><li>정답 일치와 JSON 형식 검사는 서로 다른 품질 기준입니다.</li><li>effort가 높아도 정답은 보장되지 않습니다.</li><li>데모 수치는 실제 모델 사용량이 아닙니다.</li><li>중단·시간 초과로 사용량이 누락되면 절감률을 확정하지 않습니다.</li></ul></section></div>}
